	...
	if (typeof oData == "undefined" || !oData) {
		...
		for(var k in oData) {
			...
			if (v instanceof Array || v instanceof jindo.$A) {
				$A(v).forEach(function(value,index,array) {
...
/**
 * appendTo 메서드는 인수로 지정한 $Element 객체의 마지막 자식 노드로 추가한다.
 * @param {$Element | HTMLElement | String} oElement 추가할 DOM 엘리먼트, 혹은 엘리먼트의 id
...
jindo.$Template = function(str) {
    ...
    if(typeof str == "undefined") str = "";
    else if( (obj=jindo.$(str)) && obj.tagName && (tag=obj.tagName.toUpperCase()) && (tag == "TEXTAREA" || (tag == "SCRIPT" && obj.getAttribute("type") == "text/template")) ) {
            ...
            tpl[i] = tpl[i].replace(jindo.$Template.pattern, function(){
				...
				if (m[10]) {
					return m[10].replace(/(\w+)=(?:"(\w+)"=|'(\w+)'|([0-9]+)|(\w+))/g, function(){
								var mm = arguments;
								var str = "d."+mm[1]+"=";
								if(mm[2]||mm[3]||mm[4]) str+= "\""+(mm[2]||mm[3]||mm[4])+"\"";
								else str+= "d."+mm[5];
								return str;}) +	";";
				}
				// js 
